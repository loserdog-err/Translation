...
public class NativeImageLoader
{
	 ...
	 * @param callBack   缓存回调
	 */
	public void setAvatarCache(final List<String> userIDList, final int length, final
	cacheAvatarCallBack callBack)
	{
		...
		for (final String userID : userIDList)
		{
			...
			if (userID.equals(JMessageClient.getMyInfo().getUserName()))
			{
				...
				{
					Bitmap bitmap = BitmapLoader.getBitmapFromFile(file.getAbsolutePath(), length,
							length);
					if (null != bitmap)
					{
				...
				JMessageClient.getUserInfo(userID, new GetUserInfoCallback(false)
				{
					...
					public void gotResult(int i, String s, UserInfo userInfo)
					{
						if (i == 0)
						{
							...
							if (file != null)
							{
								Bitmap bitmap = BitmapLoader.getBitmapFromFile(file
										.getAbsolutePath(), length, length);
								addBitmapToMemoryCache(userID, bitmap);
							} else
							{
//                                Bitmap bitmap = BitmapLoader.getBitmapFromFile(getR.drawable
// .head_icon, length, length);
							}
	 ...
	 * @param callBack
	 */
	public void setAvatarCache(final String userName, final int length, final cacheAvatarCallBack
			callBack)
	{
		final Handler handler = new Handler()
		{
	 ...
	 * @return
	 */
	public Bitmap loadNativeImage(final String path, final int length, final NativeImageCallBack
			callBack)
	{
		//先获取内存中的Bitmap
	 ...
	 * @return
	 */
	public Bitmap loadNativeImage(final String path, final Point point, final NativeImageCallBack
			mCallBack)
	{
		//先获取内存中的Bitmap
		Bitmap bitmap = getBitmapFromMemCache(path);
		...
		if (bitmap == null)
		{
			mImageThreadPool.execute(new Runnable()
			{
				...
				{
					//先获取图片的缩略图
					Bitmap mBitmap = decodeThumbBitmapForFile(path, point.x == 0 ? 0 : point.x,
							point.y == 0 ? 0 : point.y);
