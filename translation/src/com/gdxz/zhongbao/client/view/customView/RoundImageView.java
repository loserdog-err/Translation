...
public class RoundImageView extends ImageView
{
	...
	{
		TypedArray types = this.mContext.obtainStyledAttributes(attrs, R.styleable
				.roundedimageview);
		mBorderThickness = types.getDimensionPixelSize(R.styleable
				.roundedimageview_border_thickness, DEFAUTL_BORDER_THICKNESS);
		mBorderInsideThickness = types.getDimensionPixelSize(R.styleable
				.roundedimageview_border_inside_thickness, DEFAUTL_BORDER_THICKNESS);
		mBorderOutsideThickness = types.getDimensionPixelSize(R.styleable
				.roundedimageview_border_outside_thickness, DEFAUTL_BORDER_THICKNESS);
		...
		mBorderInsideColor = types.getColor(R.styleable.roundedimageview_border_inside_color,
				DEFAULT_COLOR);
		mBorderOutsideColor = types.getColor(R.styleable.roundedimageview_border_outside_color,
				DEFAULT_COLOR);
	...
	protected void onDraw(Canvas canvas)
	{
		...
		if (b != null)
		{
			...
			// drawInsideBorder
			drawCircleBorder(canvas, cx, cy, radius + mBorderInsideThickness / 2f,
					mBorderInsideColor, mBorderInsideThickness);
			// drawOutSideBorder
			drawCircleBorder(canvas, cx, cy, radius + mBorderInsideThickness +
					mBorderOutsideThickness / 2f, mBorderOutsideColor, mBorderOutsideThickness);
			Bitmap roundBitmap = getCroppedRoundBitmap(bitmap, radius);
			canvas.drawBitmap(roundBitmap, mDefaultWidth / 2f - radius, mDefaultHeight / 2f -
					radius, null);
		}
	 ...
	 * Draw circle border
	 */
	private void drawCircleBorder(Canvas canvas, float cx, float cy, float radius, int color, int
			thickness)
	{
		Paint paint = new Paint();
		/* Anti-aliasing */
